...
function logDetectedPlatform(respuesta, context) {
  ...
  console.log("⚙️ [EXTRACTOR] Plataforma: " + platform + " | Tipo: " + type);
}
...
function verifyYoutube(root, respuesta, context) {
  ...
  if (codeElement && aElement && emailElement) {
    ...
    if (code?.match(regexSixNumberMax) &&
      ...
      context.to = email;
      console.log(`▶️ [YOUTUBE] Destinatario detectado en el contenido | Correo: ${email}`)
...
(function () {
  ...
  if (getEnvironment() === "GAS") {
    ...
    console.log("⚙️ [EXTRACTOR] Procesamiento de enlaces omitido | Motivo: entorno sin GAS");
  }
})();
...
function main(e) {
  ...
  try {
    ...
    console.error(`⚙️ [EXTRACTOR] Error en el proceso principal | Detalle: ${err.message}`);
